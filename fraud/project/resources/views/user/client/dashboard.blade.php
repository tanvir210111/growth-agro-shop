...
@section('meta')
    <title>ড্যাশবোর্ড — {{ $gs->title }}</title>
@endsection
            ...
            <div class="col-lg-9">
                ...
                <div class="ud-hero">
                    ...
                    <div class="ud-hero-actions">
                        <a href="{{ route('user.fraud.index') }}" class="btn-w"><i class="fas fa-search"></i> ফ্রি সার্চ করুন</a>
                        <a href="{{ route('user.fraud.logs') }}" class="btn-ghost-w"><i class="fas fa-history"></i> হিস্ট্রি দেখুন</a>
                    </div>
                </div>
                ...
                <div class="ud-stats">
                    <div class="ud-stat">
                        ...
                        <div class="val">{{ number_format($data['fraud_total'] ?? 0) }}</div>
                    ...
                    <div class="ud-stat">
                        ...
                        <div class="val">
                            {{ (int) ($data['fraud_today'] ?? 0) }}
                            @if(($data['fraud_daily_limit'] ?? 0) > 0)
                                <small style="font-size:.9rem;color:#94a3b8;font-weight:600">/ {{ $data['fraud_daily_limit'] }}</small>
                            @endif
                        </div>
                ...
                <div class="ud-grid2">
                    ...
                    <div class="ud-card">
                        <div class="ud-card-h">
                            ...
                            <a href="{{ route('user.fraud.logs') }}" class="btn-cd-ghost" style="min-height:2.2rem;padding:.4rem 1rem;font-size:.8rem">সব দেখুন</a>
                        </div>
                        <div class="ud-card-b" style="padding:0">
                            @forelse(($data['fraud_recent'] ?? collect()) as $row)
                                <div class="d-flex align-items-center justify-content-between px-3 py-3" style="border-bottom:1px solid #f1f5f9">
                                    <div>
                                        <code style="font-weight:700;color:#0f172a;font-size:.95rem">{{ $row->phone }}</code>
                                        <div class="small text-muted mt-1">{{ \Carbon\Carbon::parse($row->created_at)->format('d M, Y h:i A') }}</div>
                                    </div>
                                    <div class="text-end">
                                        <span class="ud-badge ok">Success {{ number_format($row->success_ratio, 0) }}%</span>
                                        <div class="small text-muted mt-1">Cancel {{ number_format($row->cancel_ratio, 0) }}%</div>
                                    </div>
                                </div>
                            @empty
                                <div class="ud-empty">
                                    ...
                                    এখনো কোনো চেক নেই।
                                    <div class="mt-3">
                                        <a href="{{ route('user.fraud.index') }}" class="btn-cd btn-cd-primary">প্রথম চেক করুন</a>
                                    </div>
                                </div>
                            @endforelse
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="ud-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="ud-oid">#{{ $order->order_number }}</td>
                                    <td>{{ \Carbon\Carbon::parse($order->created_at)->format('d M, Y') }}</td>
                                    <td><i class="fas fa-wallet me-1 text-muted"></i>{{ $order->payment_method }}</td>
                                    ...
                                    <td class="text-end">
                                        <button type="button" class="btn btn-sm btn-primary rounded-pill px-3 view-invoice-btn"
                                            ...
                                            data-date="{{ \Carbon\Carbon::parse($order->created_at)->format('d M, Y h:i A') }}"
                                            data-method="{{ $order->payment_method }}"
                                            data-total="{{ number_format($order->total_amount) }}"
                                            data-paid="{{ number_format($order->paid_amount ?? 0) }}"
                                            data-due="{{ number_format($order->due_amount ?? 0) }}"
                                            data-desc="{{ $order->description ?? '—' }}"
                                            data-link="{{ route('frontend.invoice.live', ['order_number' => $order->order_number, 'token' => $order->hash_token ?? '']) }}"
                                        >
                    ...
                    <div class="d-flex justify-content-center py-3">
                        <nav>
                            <ul class="pagination ud-pagination mb-0">
                                ...
                                @else
                                    <li class="page-item"><a class="page-link" href="{{ $paginator->previousPageUrl() }}"><i class="fas fa-chevron-left" style="font-size:11px"></i></a></li>
                                @endif
                                ...
                                @if ($paginator->hasMorePages())
                                    <li class="page-item"><a class="page-link" href="{{ $paginator->nextPageUrl() }}"><i class="fas fa-chevron-right" style="font-size:11px"></i></a></li>
                                @else
